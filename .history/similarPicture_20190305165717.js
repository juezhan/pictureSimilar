...
async function averageData (imgList) {
  ...
  const averageList = list.map((item, index) => {
    if ((index+1)%4 === 0) {
      const item1 = item[index-3]
      const item2 = item[index-2]
      const item3 = item[index-1]
      const item4 = item[index]
      const average = (item1 + item2 + item3 + item4)/4;
      return average
    }
  });
